...
@section('content')
    <div class="mb-6">
        <h1 class="text-3xl font-semibold text-[#191c1d]">Stock Adjustment Report</h1>
        <p class="mt-1 text-sm text-[#3e494a]">Manual and stock count adjustment ledger rows.</p>
    </div>
        ...
        <table class="w-full text-left text-sm">
            <thead class="bg-[#f3f4f5] text-xs uppercase tracking-[0.06em] text-[#3e494a]">
                <tr>
                    ...
                    <th class="px-5 py-3">Source Stock Count</th>
            ...
            <tbody class="divide-y divide-[#bec8ca]">
                @forelse ($ledgers as $ledger)
                    <tr>
                        ...
                        <td class="px-5 py-4 text-[#3e494a]">{{ $ledger->reference_type === \App\Models\StockCount::class ? '#'.$ledger->reference_id : '-' }}</td>
                        <td class="px-5 py-4 text-[#3e494a]">{{ $ledger->creator?->name ?: '-' }}</td>
                    </tr>
                @empty
                    <tr><td colspan="9" class="px-5 py-8 text-center text-[#3e494a]">No adjustment rows found.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
