...
@section('title', $category->exists ? 'Edit Expense Category' : 'New Expense Category')
@section('page-title', 'Expense Categories')

@section('content')
    <div class="mb-6">
        <h1 class="text-3xl font-semibold text-[#191c1d]">{{ $category->exists ? 'Edit Expense Category' : 'New Expense Category' }}</h1>
        <p class="mt-1 text-sm text-[#3e494a]">Inactive categories stay in history but cannot be used for new expense entries.</p>
    </div>

    <form method="POST" action="{{ $category->exists ? route('finance.expense-categories.update', $category) : route('finance.expense-categories.store') }}" class="max-w-2xl rounded-lg border border-[#bec8ca] bg-white p-6">
        @csrf
        @if ($category->exists)
            @method('PUT')
        @endif
